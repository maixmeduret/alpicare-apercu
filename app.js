...
(function () {
  ...
  /* ---------- 1 bis. Le simulateur et le tableau des garanties suivent le réglage ---------- */
  function propager() {
    var f = $('#simu-form');
    if (f) {
      var champs = { age: etat.age, regime: etat.regime, adultes: etat.adultes, enfants: etat.enfants };
      var change = false;
      Object.keys(champs).forEach(function (n) {
        var i = f.querySelector('input[type="hidden"][name="' + n + '"]');
        if (i && i.value !== String(champs[n])) { i.value = String(champs[n]); change = true; }
      });
      if (change) f.dispatchEvent(new Event('change', { bubbles: true }));
    }
    $$('[data-regime-radio]').forEach(function (r) { r.checked = (r.value === etat.regime); });
    $$('[data-regime-libelle]').forEach(function (e) {
      e.textContent = e.getAttribute('data-libelle-' + etat.regime) || e.textContent;
    });
    $$('[data-suisse]').forEach(function (e) {
      var t = e.getAttribute('data-' + etat.regime);
      if (t) e.textContent = t;
    });
  }
  var rendreSeul = rendre;
  rendre = function () { rendreSeul(); propager(); };

  /* Le choix de régime du tableau des garanties pilote aussi le réglage */
  $$('[data-regime-radio]').forEach(function (r) {
    r.addEventListener('change', function () {
      if (r.checked && r.value !== etat.regime) { etat.regime = r.value; rendre(); }
    });
  });

  /* Un lien vers une section repliée l'ouvre */
  function ouvrirRepli() {
    var id = location.hash.slice(1);
    if (!id) return;
    var c = document.getElementById(id);
    while (c) {
      if (c.tagName === 'DETAILS') { c.open = true; break; }
      c = c.parentElement;
    }
  }
